...
- `connect [devices|filter|pick]` - Connect to devices by name, filter (`model=`, `site=`, `role=`), `all`, or the current selection if omitted (already-connected devices are skipped, so bare `connect` retries only devices that failed last time). `connect pick` opens an interactive picker: arrow keys to move, space to toggle a device, `a`/`c` to select all/clear, Enter to confirm and connect, `q`/Ctrl+C to cancel
